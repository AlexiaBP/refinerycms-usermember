...
Next, to install the members plugin run:
    ...
    rails generate refinery:usermembers

Note that the above task copies `*.refinery_usermembers.rb` migration files to your `db/migrate` directory.
You should examine `*_create_usermembers.refinery_usermembers.rb` file and adjust it (if necessary)
...
> and add this gem later, or rename `*.refinery_usermembers.rb` migrations to follow
